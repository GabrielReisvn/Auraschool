...
public class ServidorHttpSimples {
    ...
    private static void login(HttpExchange t) throws IOException {
        if (!t.getRequestMethod().equalsIgnoreCase("POST")) {
            enviarArquivo(t, "index.html", "text/html");
            return;
        }

        String corpo = ler(t); // Exemplo: tipo=produtor&user=...&senha=...
        corpo = URLDecoder.decode(corpo, StandardCharsets.UTF_8);

        String user = pega(corpo, "user");
        String senha = pega(corpo, "senha");
        String opcao = pega(corpo, "opcoes");  // "aluno" ou "professor"
        ...
        if (user.equals("ana") && senha.equals("123") && opcao.equals("aluno")) {
            ...
            enviarArquivo(t, "index.html", "text/html"); // Reenvia o login com erro
        }
    }

    // -------------------- PRODUTOR (Cadastro de atividades) --------------------
    ...
    private static void produtor(HttpExchange t) throws IOException {
        ...
        private static void consumidor (HttpExchange t) throws IOException {
            ...
            try (Statement st = con.createStatement();
                ...
                while (rs.next()) {
                    ...
                    System.out.println("dados:" + nome + desc + data + curtida);
                    ...
                    html.append("<div class=\"container-dina").append(classeExtra).append("\">");
        ...
        private static void enviarArquivo (HttpExchange t, String arquivo, String tipo) throws IOException {
            File f = new File("src/main/java/src/" + arquivo);

            byte[] bytes = java.nio.file.Files.readAllBytes(f.toPath());
            t.getResponseHeaders().add("Content-Type", tipo + "; charset=UTF-8");
        ...
        private static void enviar (HttpExchange t, String arq) throws IOException {
            File f = new File("src/main/java/src/" + arq);
